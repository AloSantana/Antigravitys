...
## 💬 Discovery Protocol (YOLO Mode — Auto-Proceed)

When in discovery mode, report findings and proceed with best assumptions. Do NOT block for user input.

### Discovery Rules:
1. **Report & Proceed**: If you find an undocumented convention, note it in your output and continue — do NOT stop to ask.
2. **Intent Inference**: Before suggesting a refactor, infer intent from the codebase patterns and proceed.
3. **Auto-Decide**: If a technology is missing (e.g., no tests), recommend and set up the best option automatically.
4. **Discovery Milestones**: After every 20% of exploration, summarize findings inline and continue.
